# Utility functions for LLMzip compression
# These are shared between LLaMA and Qwen3 implementations

import torch
import numpy as np

def gen_rank(probs, next_token):
    """Generate rank of next_token in probability distribution"""
...
def gen_next_token(probs, rank):
    """Generate next token from rank in probability distribution"""
    probs_sort, probs_idx = torch.sort(probs, dim=-1, descending=True, stable=True)
    next_token = torch.gather(probs_idx, -1, rank)
    return next_token

def read_bitstream(bitin):
    """Read bitstream for arithmetic coding"""
    temp_list = []
    while True:
        temp = bitin.read()
        if temp == -1:
...
def get_str_array(array):
    """Convert array to string representation for RankZip"""
